...
var Quintus = function Quintus(opts) {
    ...
    // where images, sound and other data files should be found
    ...
    if(!!window.wish && wish.assetsPath){
        assetsPath = wish.assetsPath + '/game/';
    }
    ...
    Q._needForceRepaint = true;

    (function(){
        var userAgent = navigator.userAgent;

        //不需要强刷的机器.
        var isIOS = (/iPhone|iPod|iPad|iTouch/i).test(userAgent);

        //米3强刷canvas会被挂起.
        var mi3 = (/MI 3 Build/i).test(userAgent);

        if(isIOS || mi3){
            Q._needForceRepaint = false;
        }
    ...
    Q.gameLoop = function (callback) {
        ...
        Q.gameLoopCallbackWrapper = function () {
            ...
            //force repaint
            if(Q._needForceRepaint){
                Q.el.style.display='none';
                Q.el.offsetHeight; // no need to store this anywhere, the reference is enough
...
//        Q.cssHeight = h;
        ...
        Q.width = parseInt(Q.el.width, 10);
        Q.height = parseInt(Q.el.height, 10);
        Q.cssWidth = cssWidth || w;
        Q.cssHeight = cssHeight || w;
    ...
    Q.assetType = function (asset) {
        ...
        // Use the web sound loader instead of the regular loader
        // if it's supported.
    ...
    // List of mime types given an sound file extension, used to
    ...
    Q.loadAssetAudio = function (key, src, callback, errorCallback) {
        ...
        /* No supported sound = trigger ok callback anyway */
        if (!extension) {
            callback(key, null);
            return;
        }
